...
- [ ] 用户输入以 `› ` 回显在对话区;模型正文逐 token 到达时**续在同一行**(不得一行一个词);超宽自动折行

### 已知限制

- 对话区按**逻辑行数**估算可视窗口(不按折行后的实际行数):单条极长且无换行的回复会滚动到该行中部;本轮不做滚动缓冲/翻页。
- TUI 模式下子 agent 的活动行(`⤷ [子N] 派生` / `● [子N] 工具`)不显示:`UiEvent` 没有子事件变体,而 `ChildEvent` 钩子在 `build_loop` 期已按纯文本渲染器装配;TUI 模式下该钩子被置空以免污染画面。要在 TUI 里呈现需给事件模型加子事件变体。
- `PendingDetail::Other` 类动作(如 `spawn_subagent`)的弹层只显示 `summary` 单行,无结构化明细。
