...
类 Claude Code 的 CLI 编程 agent(Rust)。两大差异化:可插拔多 provider(Anthropic 格式 + OpenAI 兼容格式)、DeepSeek 前缀缓存优化。Phase 1(Anthropic 闭环)、Phase 2(OpenAI 兼容 adapter)、Phase 3(三级权限 + grep_search/todo_write + 只读并发)、Phase 4(AGENTS.md 注入 + 上下文压缩 + 前缀缓存校验/遥测)均已完成并真实联调;Phase 5(错误边界、可观测性、配置文档)已完成;Phase 6 模块一(sub-agent 派生机制)、模块二(ratatui TUI 全屏界面)均已完成,见路线图。此外 `/settings` 设置页已全功能落地(REPL + TUI 双入口;TUI 支持编辑写回 lex-code.toml 并热生效)。
...
- `lex-cli/src/` — 渲染与 UI;错误用 anyhow,提示词/交互输出用中文,anstream 输出 ANSI。确认与 REPL **共享同一个** `CliInput`(stdin BufReader 分开会丢预读输入)。UI 硬约束:所有颜色/ANSI 序列只从 `ui/theme.rs` 取,其他文件禁止裸 `\x1b[`;raw mode 期间换行必须显式 `\r\n`(`\n` 不回车,输入盒边框会错位);流式正文经 `ui/markdown.rs` 行缓冲渲染(粗体/斜体/行内代码/标题/列表/围栏,纯函数有单测,改渲染先跑 `editor_tests`/markdown 测试);提交输入盒 = 折叠(留 `› 回执`),退出 = 整盒清除。`ui/settings.rs` 是 `/settings` 设置页:`parse_command` 斜杠分发(唯一前缀命中,如 `/setting`)→ `SettingsView` 只读快照 → 渲染/状态机纯函数可单测;非 TTY 自动降级线性摘要;Key 只显示"环境变量 LEX_*",绝不回显;按键复用 `input.rs` 的 `event_bus()`/`RawGuard`。`tui/` 是 ratatui 全屏交互界面(`diff`/`event`/`markdown`/`state`/`draw`/`confirm`/`run`,交互终端下默认启用,`--plain` 与非 TTY 自动降级到上述纯文本路径);对话正文按行做 TUI 版 markdown 渲染(`tui/markdown.rs`,规则对齐 `ui/markdown.rs`:标题加粗/列表蓝点/围栏内容变暗带底色 `C_CODE_BG`/行内代码亮青/表格整块缓冲按 CJK 显示宽度对齐输出;`MarkdownState` 跨行持有,流式行关闭时才整行渲染,表格块只在轮次边界落定——行间关闭传 `close_open_text(false)` 防止冲掉未写完的表);PageUp/PageDown/End/鼠标滚轮回看对话(`AppState.scroll` 按显示行计,状态行提示偏移,新输入跳回底部;**视口必须按折行后的显示高度切片**——按逻辑行数取窗口会让折行溢出面板底边,回复尾部/回显被裁到下次提交才可见,踩过这个坑);待办面板手工按显示宽度折行(图标与内容首段固定同行、续行悬挂缩进——交给 Paragraph 会把行尾图标折成孤立行);输入框终端光标常驻在输入文本末尾(`Frame::set_cursor_position`,CJK 按显示宽度计);斜杠命令 `/settings` `/compact` `/init` 在 REPL 与 TUI 双路径同口径分发(`ui/settings.rs::parse_command` 注册,前缀唯一命中):/settings 见下;/compact 手动触发压缩(`AgentLoop::compact_now`,与自动压缩共用切分与摘要逻辑,但不受阈值与"会话内只一次"限制,成功同样置位 + 失效缓存,摘要并入下一条用户消息);/init 把内置提示词(`init_prompt()`)替换用户输入走正常对话路径驱动模型生成/完善 AGENTS.md。输入框是完整行编辑:`AppState.cursor` 按字符计,←/→/Home/End 移动(End 因此让位,不再回底)、光标处插入/删除、Backspace/Delete/CJK 均按字符;终端光标常驻输入框(`Frame::set_cursor_position`);斜杠命令补全:输入 "/" 且未进参数区(原始输入含空格即关闭)时,输入栏下方弹两行候选菜单(选中强调蓝,↑/↓ 循环、Tab 补全、Enter 执行,候选由 `completion_candidates` 实时推导);输入栏下方常驻上下文容量行(`ContextUsage{used,limit}` 事件,agent 侧在每轮前后用 `estimate_tokens` 上报,逼近 80% 阈值转警示色,未启用压缩只显示估算);待办图标自绘三态 `◉`(绿,完成)/`◐`(蓝,进行中)/`○`(灰,待办);TUI 硬约束:**颜色只取 `theme.rs` 的 `ratatui::style::Color` 常量(`C_*`),同样禁止裸 `\x1b[`**;**确认弹层必须展示命令原文或 diff,禁止"是否继续?"式笼统提示;内容超长时内容区可滚动(`confirm_scroll`,底部 `[y]/[n]` 提示行固定并带 `{scroll}/{max}` 指示;弹层为最上层时 PgUp/PgDn/滚轮只滚弹层,打开时归零)**;`/settings` 设置页全功能:`SettingsView` 携带 `config` 快照,详情页字段可编辑——模块 0 provider/model/base_url/max_tokens/thinking/effort(Enter 进入编辑,输入框临时接管为字段编辑器:主输入暂存 `input_stash`、Esc 取消恢复),模块 1 追加三级规则(硬约束:只能追加),模块 2 limit/enabled,模块 3 只读;字段保存 = `apply_edit`(校验 + 改写 config + `write_back` 读-改-写 lex-code.toml,基于 `toml::Table` 保留未知字段、注释不保留)+ 排队 `settings_commit` → `UiCommand::HotApply` → agent 侧热生效(rebuild provider / 换 SecurityRules / 改 context_limit + 失效缓存;共享 cfg 同步,子 agent 的 provider 重启才同步);`run_tui` 的 cfg 为 `Arc<Mutex<Config>>` 且接收 `build_throttled_provider` 函数指针;`run_tui` 的待办句柄**只能从 `agent.tool_ctx.todos` 派生**(另建 `Arc` 会让面板永远空白且单测发现不了);TUI 模式下 `build_loop` **不得注入纯文本渲染钩子**(工具/子 agent 活动行会往 stdout 打 ANSI 破坏画面);状态变更只在 UI 线程发生(`AppState::apply`),渲染与 agent 事件流经 channel 解耦。
...
- ~~Phase 5:错误边界打磨、可观测性、配置文档~~(已完成:provider 客户端 connect_timeout 防无界阻塞;`execute_tool_call` 记录工具耗时/结果日志;日志级别 `LEX_LOG` > `RUST_LOG` > warn(TUI 全屏模式改写 `%TEMP%/lex-code-tui.log`——stderr 会砸坏 ratatui 画面,踩过"WARN 画进输入框"的坑;其余模式仅 stderr,不污染渲染);根目录 `README.md` 覆盖配置全字段、环境变量、安全模型、日志事件表)。
...
- `/settings` 设置页(已完成,全功能):规格 `docs/superpowers/specs/2026-09-16-settings-page-design.md`,计划 `docs/superpowers/plans/2026-09-16-settings-page.md`;REPL 只读 + TUI 编辑写回 lex-code.toml 并热生效(provider/安全规则/上下文限制;max_children 等个别项重启生效)。
