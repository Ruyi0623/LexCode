...
- 开工前核对时测试总数**实为 140**(Global Constraints 里写的 111、L1273 的「≥115」均为旧数),当时基线全绿。
- **模块一全部完成后(2026-09-17,Task 1~8 收尾)测试总数为 158**:140 + Task 1~7 新增单测 + Task 8 跨层回归 `lex-core/tests/subagent_e2e.rs`(2 个:主循环派发只回摘要、Forbidden 规则在子 agent 内仍拦截)。以 158 作为模块二(TUI)开工的实际基线。
